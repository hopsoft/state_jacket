...
module StateJacket
  ...
  class TransitionSystem
    ...
    def lock
      ...
      @rules.values.each { |target_list| target_list&.freeze }
      @rules.freeze
    ...
    # If `keys` is `nil` or empty, a full hash representation (same as `to_h`) is returned.
    #
    ...
    # This includes lock status, all states, transitioner states, terminal states, and the complete rule map.
    #
    ...
    def overlaps?(rule)
      ...
      from.any? do |origin|
        next false unless @rules.key?(origin)

        destinations = @rules[origin]
        next false unless destinations

        to.any? { destinations.include? it }
      end
    end
    ...
    # @rbs return: bool
    def terminal?(state)
      return false unless includes?(state)
