...
class StateJacket::PatternMatchingTest < Test
  class MachineStateMatchingTest < Test
    ...
    def test_machine_pattern_matching_with_additional_attributes
      # Test pattern matching with additional attributes beyond current_state
      cart_machine = @machine

      submitted_machine = StateJacket::StateMachine.new(@machine.transition_system, current_state: :submitted)
      submitted_machine.on :pay, submitted: :paid
      submitted_machine.on :cancel, submitted: :cancelled
      submitted_machine.lock

      shipped_machine = StateJacket::StateMachine.new(@machine.transition_system, current_state: :shipped)
    ...
    def handle_order_state(machine)
      case machine
      in { current_state: "cart" }
        "Order is in cart state"
      in { current_state: "submitted" }
        "Order has been submitted"
      in { current_state: "paid" }
        "Order has been paid"
      in { current_state: "shipped" }
        "Order has been shipped"
      in { current_state: "delivered" }
  ...
  class TransitionResultMatchingTest < Test
    def setup
      system = StateJacket::TransitionSystem.new
      system.add cart: [:submitted, :abandoned]
      system.add submitted: [:paid, :cancelled]
    ...
    def test_transition_result_pattern_matching
    ...
    def test_transition_result_pattern_matching_with_specifics
    ...
    def handle_order_transition(result)
      ...
      in { event: "pay", status: :ok }
        "Payment processed successfully"
      in { event: "pay", status: :error, error: error }
        "Error during payment: #{error.message}"
      in { status: :error, error: error }
        "Error: #{error.message}"
      else
        "Unknown transition result"
      end
    end
  end
  ...
  class PatternMatchingWithGuardClausesTest < Test
    def setup
      ...
      @user_machine = StateJacket::StateMachine.new(system, current_state: :pending)
    ...
    def determine_user_permissions(machine, admin:)
      case machine
      in { current_state: "active" } if admin
        "Full access"
      in { current_state: "active" }
        "Edit and view"
      in { current_state: "pending" } | { current_state: "locked" } if admin
        "View only"
      in { current_state: "pending" }
        "View only"
      else
        "No access"
      end
  ...
  class BuiltInPatternClassesTest < Test
    ...
    def test_transition_result_matching_in_compound_patterns
    ...
    def test_transition_system_pattern_matching
      # Test pattern matching on the transition system
      result = case @system
